 ...
 */
  ...
  $next = db_query("SELECT nid FROM node 
  					WHERE type = 'home' AND nid > :nid AND nid <> :nid 
  ...
  <?php
  $thumbs = '<div class="thumb-containers"><div class="thumbs">';
  ...
  if ($sql):
    ...
    // var_dump($result);
    $imageSet		= $result[0];
    for ($i = 0, $a = 1, $n = 1; $i < count($imageSet); $i++, $a++, $n++):
      $r			= $imageSet[$i];
      $fid			= $r->fid;
	  ...
	  ?>
	    <figure id="image<?php print $a; ?>"><img src="<?php print $image; ?>" alt=""></figure>
	    <?php
	    if ($a === count($result)) {
          $thumbs .= '<a href="#image' . $a . '" id="' . $a . '"><img src="' . $thumb . '" alt="" /></a>';
          $thumbs .= '</div>';
	    }
	    if (in_array($a, $fives)) {
          $thumbs .= '<a href="#image' . $a . '" id="' . $a . '"><img src="' . $thumb . '" alt="" /></a>';
